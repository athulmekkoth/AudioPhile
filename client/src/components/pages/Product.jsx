    ...
    {data ? (
      <div className="">
        ...
        <div className="mt-20 lg:mt-96">
          <div className="flex text-2xl flex-row justify-evenly items-center mx-3 my-5 py-4 flex-wrap">
            <div className="flex flex-row gap-10 bg-gray-400 p-4 rounded-lg">
              <p>Select qty:</p>
              <span onClick={() => (values >= 4 ? alert('Maximum 4 per order') : setvalue((values) => values + 1))}>
                <FaPlus />
              </span>
              <p>{values}</p>
              <span onClick={() => (values > 1 ? setvalue((values) => values - 1) : alert('Minimum order is 1'))}>
                <FaMinus />
              </span>
            </div>
            {data.count > 0 ? (
              <button
                onClick={() => (values === 0 ? notification('Values must not be zero', 'warn') : add(data, values))}
                className="w-[70%] mt-3 py-2 bg-orange-500 rounded-lg"
              >
                Add to cart
              </button>
            ) : (
              <button className="w-[70%] mt-3 py-2 bg-orange-500 disabled rounded-lg">
                Out of stock
              </button>
            )}
          </div>
          <div className="flex flex-col items-center gap-3 lg:gap-0 lg:flex-row w-[90%] mx-auto lg:justify-around text-2xl">
            <p className="overflow-auto text-center text-3xl">Price: ${data.price}</p>
        ...
        <div className="flex flex-col gap-6 items-center mx-10 my-10 lg:flex-row justify-between">
          <div>
            <h1 className="text-center text-orange-300 text-2xl">What's in the Box</h1>
          ...
          <div className="w-[70%] lg:w-[40%]">
            ...
            <p>
              Your satisfaction is our top priority, which is why we stand behind the quality of our earphones with a
              comprehensive warranty. Our warranty covers any manufacturing defects or malfunctions that may occur within a specified period from the date of purchase. In the unlikely event that your earphones experience any issues, simply contact our customer service team, and we will be happy to assist you.
              <br />
              <span className="font-bold text-orange-400">For any query: <br /> support@audiophile.com.</span>
            </p>
          </div>
        </div>
      </div>
